...
void UVK::VKDepthBuffer::createDepthBufferImage(Swapchain& swapchain, VKDevice& device) noexcept
{
    ...
    depthImage.createImage({ swapchain.getExtent().width, swapchain.getExtent().height }, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, bufferImageMemory, device);
