...
void UVK::EditorPawn::beginPlay()
{
    ...
    if (!global.getEditor())
        Logger::log("You are currently using an Editor Pawn ingame. This is a safety measure in order to prevent instant crashes of your game, however, the code for the EditorPawn will not compile in production which will cause a crash!", UVK_LOG_TYPE_ERROR);

    coreCache = &actor.get<CoreComponent>();
    coreCache->translation = { 0.0f, 0.0f, 0.0f };
