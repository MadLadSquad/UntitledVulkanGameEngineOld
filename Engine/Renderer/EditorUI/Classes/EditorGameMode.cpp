...
{
    if (!global.getEditor())
        Logger::log("You are currently using an Editor Game Mode ingame. This is a safety measure in order to prevent instant crashes of your game, however, the code for the EditorGameMode will not compile in production which will cause a crash!", UVK_LOG_TYPE_ERROR);
